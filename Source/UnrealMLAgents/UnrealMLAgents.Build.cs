...
public class UnrealMLAgents : ModuleRules
{
	...
	public UnrealMLAgents(ReadOnlyTargetRules Target)
		: base(Target)
	{
	...
	private bool ConfigurePlatform(string Platform, UnrealTargetConfiguration Configuration)
	{
		...
		foreach (var lib in GrpcLibs)
		{
			...
			{
				string fullPath = root + "grpc/" + "lib/" + CommunicatorPlatformInstance.LibrariesPath + arch + CommunicatorPlatformInstance.ConfigurationDir(Configuration) + CommunicatorPlatformInstance.LibraryPrefixName + lib + CommunicatorPlatformInstance.LibraryPostfixName;
		...
		foreach (var lib in ProtobufLibs)
		{
			foreach (var arch in CommunicatorPlatformInstance.Architectures())
			{
				...
				string fullPath = root + "protobuf/" + "lib/" + CommunicatorPlatformInstance.LibrariesPath + arch + CommunicatorPlatformInstance.ConfigurationDir(Configuration) + libPrefixName + lib + CommunicatorPlatformInstance.LibraryPostfixName;
				PublicAdditionalLibraries.Add(fullPath);
			}
		}
		// abseil
		foreach (var lib in AbseilLibs)
		{
			foreach (var arch in CommunicatorPlatformInstance.Architectures())
			{
				string fullPath = root + "abseil/" + "lib/" + CommunicatorPlatformInstance.LibrariesPath + arch + CommunicatorPlatformInstance.ConfigurationDir(Configuration) + CommunicatorPlatformInstance.LibraryPrefixName + lib + CommunicatorPlatformInstance.LibraryPostfixName;
				PublicAdditionalLibraries.Add(fullPath);
			}
		}
		// re2
		foreach (var lib in Re2Libs)
		{
			foreach (var arch in CommunicatorPlatformInstance.Architectures())
			{
				string fullPath = root + "re2/" + "lib/" + CommunicatorPlatformInstance.LibrariesPath + arch + CommunicatorPlatformInstance.ConfigurationDir(Configuration) + CommunicatorPlatformInstance.LibraryPrefixName + lib + CommunicatorPlatformInstance.LibraryPostfixName;
				PublicAdditionalLibraries.Add(fullPath);
			}
		}
		return false;
